...
const navItems = [
  ...
  { href: '/dashboard/requests', icon: FileText, label: 'Service Requests', roles: ['CLIENT'] },
          ...
          {allowedNav.map(({ icon: Icon, label, href }) => {
            ...
            return (
              <Link
                key={label}
                to={href}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded text-[14px] transition-all duration-150 ${isActive
