...
export default function AdminLayout({ user, onLogout }: { user: any, onLogout: () => void }) {
 ...
 <p className="text-[13px] font-semibold text-[#1E3A5F] leading-none">{user?.name || 'Administrator'}</p>
 <p className="text-[11px] mt-1 leading-none text-[#6B7280]">{user?.role || 'ROOT'}</p>
 </div>
 </div>
 </header>
