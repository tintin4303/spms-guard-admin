...
export default function MapControl({ user }: { user?: any }) {
 ...
 const loadPaths = () => {
 if (selectedSiteId) {
 fetchPatrolPaths(selectedSiteId, user?.id).then(setSavedPaths).catch(console.error);
 } else {
 setSavedPaths([]);
 }
 };
 ...
 key={i} 
 draggable={!isClient} 
 onDragStart={(e) => !isClient && handleDragStart(e, i)}
 onDrop={(e) => !isClient && handleDrop(e, i)}
 onDragOver={handleDragOver}
 className={`bg-[#0F172A] border border-[#475569] rounded p-2 text-white text-[12px] ${!isClient ? 'cursor-grab active:cursor-grabbing group hover:border-[#3B82F6]' : ''} flex items-center justify-between shadow-sm relative`}
 >
 ...
 {!isClient && viewingPath && !isEditing && (
 <>
